...
team_name_lst = data['Tm'].tolist()
team_name_st = set(team_name_lst)
team_name_lst = list(team_name_st)

#Loop through the list of each team name and extract each player with
...
for x in range(len(team_name_lst)):
	...
	x_df.to_csv(f'nba_separated_{team_name_lst[x]}.csv')